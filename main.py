...
def build_face_database(detector: SCRFD, recognizer: ArcFace, params: argparse.Namespace, force_update: bool = False) -> FaceDatabase:
    ...
    # Process images directly in the faces directory
    for filename in os.listdir(params.faces_dir):
        file_path = os.path.join(params.faces_dir, filename)
        ...
        try:
            ...
            if len(kpss) == 0:
                logging.warning(f"No face detected in {file_path}. Skipping...")
                continue
    ...
    # Process subdirectories (each subdirectory name becomes the label)
    for item in os.listdir(params.faces_dir):
        item_path = os.path.join(params.faces_dir, item)
        ...
        person_name = item
        logging.info(f"Processing faces for person: {person_name}")
        
        # Process all images in this person's directory
